...
class Mysock(object):
            ...
            for r in rs:
                if r == self.soc:
                    ...
                    try:
                        ...
                        da = re.findall(r'code:sendfilestart<([\w\W]+?)><([\w\W]+?)>',data.decode())
                        if len(da) == 1:
                            da = da[0]
                            type_ = da[0]
                            da = da[1]
    ...
    def recv_file(self,soc,path,savepath=None):
        if not savepath:
            savepath = path
        soc.send(b'code:sendfilestart<send_file><%s>' % path.encode())
        data = soc.recv(1024*FILEIOSIZE)
        da = re.findall(r'<path=([\w\W]+?)><size=(\d+)><md5=([\w\W]+?)>',data.decode())
        if len(da) == 1:
            da = da[0]
            soc.send(b'code:ok')
            ...
            with open(savepath,'wb') as f:
                while True:
                    data = soc.recv(1024*FILEIOSIZE)
                    if data.endswith(('<endpath=%s><size=%s><md5=%s>' % (path, size, md5)).encode()):
                        f.write(data[:-len(('<endpath=%s><size=%s><md5=%s>' % (path, size, md5)).encode())])
                        break
                    f.write(data)
    ...
    def synchro_upload(self,soc):
        if self.isclient:
            ...
            soc.send(self.serverfile.encode())
            data = soc.recv(1024*FILEIOSIZE)
        ...
        pinfo = path_info(uploadfile,['py'])
        soc.send(pickle.dumps(pinfo))
    ...
    def synchro_download(self,soc):
        if self.isclient:
            ...
            if data != b'get_synchro_download':
                raise ValueError('错误的参数，应该是%s' % data.decode())

            basepath = self.clientfile
            updownfile = self.serverfile
            soc.send(updownfile.encode())
        else:
            soc.send(b'get_synchro_upload')
            basepath = soc.recv(1024*FILEIOSIZE).decode()
        ...
        cData = path_info(basepath,['py'])[2]
        for filename,md5 in sData.items():
            if cData.get(filename) != md5:
                print(filename,'开始发送',end='\r')
                self.recv_file(soc,os.path.join(server_basepath,filename),os.path.join(basepath,filename))
